...
void ADCClass::calcAverage() {
  int32_t adc_data, adc_min, adc_max;
  uint8_t count;
  adc_data = 0;
  adc_max = -8388608;
  adc_min = 8388607;
  ...
  if ((abs(weightAverage - prevAverage) > CHANGE_THRESHOLD * 0.5) &&
      (abs(weightAverage - oldPrev) > (CHANGE_THRESHOLD * 1))) {
...
void ADCClass::begin(uint8_t cs_pin, uint8_t drdy_pin) {
  ...
  ads1220.writeRegister(
      2,
      7 + 8 + 16 + 128);  // IDAC current at 1500uA, Low-side power switch on, 50Hz+60Hz filter on, VREF using AIN0/AIN3
  ...
  //ESP_LOGD(__FUNCTION__, "CS pin: %i", cs_pin);
  //ESP_LOGD(__FUNCTION__, "DRDY pin: %i", drdy_pin);
  Serial.printf("CS Pin: %i  DRDY pin: %i\n", cs_pin, drdy_pin);
  ...
  Serial.println("Wacht op DRDY");
  waitForDRDY();
  getAdcValue(MUX_AINP_AINN_SHORTED);
  delay(100);

  Serial.println("Start omzetting");
...
void ADCClass::waitForDRDY() {
  detachInterrupt(drdy_pin);
  while (digitalRead(drdy_pin) == true)
    delay(1);
  handleDRDY();
}
...
void ADCClass::tare() {
  ...
  for (uint8_t count = 0; count < 4; count++) {
    delay(100);
    if (count != 0)
      accum += getAverage();  // eerste resultaat weggooien
  }
...
void ADCClass::startConversion(uint8_t channel = 255, bool continuous = false) {
  ...
  if (continuous != continuousMode) {
    continuous ? ads1220.set_conv_mode_continuous() : ads1220.set_conv_mode_single_shot();
